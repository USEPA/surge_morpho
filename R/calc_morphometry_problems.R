...
#                       dest = here("data/flooded_lands_i5075nventory.gpkg"))
...
surge_morpho_problems <- readr::read_csv(here("data/all_lakes_lakemorpho.csv")) |>
  filter(is.na(maxDepth) | maxDepth == 0)
...
morph_it <- function(lake, p = function(...) message(...)) {
  ...
  result <- tryCatch({
    ...
    metrics <- list(surfaceArea = lakeSurfaceArea(lake_lm), 
                    ...
                    #majorAxis = lakeMajorAxisLength(lake_lm), 
                    #minorAxis = lakeMinorAxisLength(lake_lm), 
                    #axisRatio = lakeMinorMajorRatio(lake_lm), 
